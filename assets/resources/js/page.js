...
function playVideos(e) {
  ...
  const finishModal = `</div><button onclick="lvideoClose();" data-cursor="-opaque" class="lvideo-close"></button>`;
  ...
  if (url.indexOf("youtube") !== -1 || url.indexOf("youtu") !== -1) {
    ...
    document.getElementById(
      "lvideo-wrap"
    ).innerHTML = `${startModal}<video controls loop playsinline autoplay data-cursor="-opaque-mini"><source src='${this.dataset.url}' type="video/mp4"></video>${finishModal}`;
  } else {
